...
区块      可见 ${c.visible}  缓存 ${c.cached}  生成中 ${c.generating}  待上传 ${c.uploadsPending}
...
三角形    近景 ${(s.worldTriangles / 1000).toFixed(0)}k  覆盖图 ${(s.overviewTriangles / 1000).toFixed(0)}k（${Math.round(s.overview * 100)}%）
`}
      {hover &&
        `—— 地形取样 ——
