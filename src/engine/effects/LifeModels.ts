...
export function fishingBoatGeometry(): THREE.BufferGeometry {
  ...
  b.box(2.35, 0.85, -0.04, 2.45, 1.6, 0.04, T.mast)
  b.box(2.25, 1.25, -0.14, 2.55, 1.55, 0.14, T.lacquer)
  b.box(-0.2, 0.7, 0.35, 0.2, 0.9, 0.7, T.straw, 0.9)
...
/**
 * 漕船（运货船）：十一格长、宽身平底，中段满载——苇席盖着的货堆、捆好的麻包、酒坛；
 * 一根桅、一面苇篾硬帆，船尾一间小篷、一支大橹与一名艄公，船头两人撑篙。内河航道上最常见的中等船。
 */
export function cargoBoatGeometry(): THREE.BufferGeometry {
  const b = V()
  b.box(-5, 0, -1.3, 5, 0.45, 1.3, T.hullDark)
  b.box(-5.5, 0.4, -1.7, 5.3, 1.05, 1.7, T.hull)
  b.box(5.2, 0.7, -1.2, 6, 1.3, 1.2, T.hull)
  ...
  b.box(-2.6, 2.35, -0.5, 2.2, 2.55, 0.5, T.thatch, 0.85)
  for (const [x, z] of [[2.6, -0.8], [2.6, 0.0], [2.6, 0.8], [3.3, -0.4], [3.3, 0.4]] as const) b.box(x - 0.3, 1.0, z - 0.32, x + 0.3, 1.6, z + 0.32, '#c8b89a')
  for (const [x, z] of [[-3.2, -0.7], [-3.2, 0.7]] as const) {
    b.box(x - 0.3, 1.0, z - 0.3, x + 0.3, 1.65, z + 0.3, '#6a4a30')
    b.box(x - 0.18, 1.65, z - 0.18, x + 0.18, 1.8, z + 0.18, '#3a2a1c')
  }
  // 桅与苇篾硬帆
  b.box(0.8, 1.0, -0.12, 1.05, 8.0, 0.12, T.mast)
  ...
  boatman(b, 4.4, 1.0, -0.8, '#7a5a3e', false)
  b.box(4.6, 0.2, 0.9, 4.72, 4.4, 1.02, T.straw)
  return b.build()
}

/**
 * 海上渔船：九格长的两桅小福船——尖首高尾，前后两面红褐硬帆，船舷挂网，船尾一名舵手。
 * 海上以大船为主，偶有几条它在近岸作业。
 */
export function seaFisherGeometry(): THREE.BufferGeometry {
  const b = V()
  b.box(-3.6, 0, -0.9, 3.6, 0.5, 0.9, T.hullDark)
  b.box(-4.2, 0.45, -1.35, 4.0, 1.3, 1.35, T.hull)
  b.box(4.0, 0.8, -0.9, 5.0, 1.8, 0.9, T.hull)
  b.box(-4.6, 0.9, -1.2, -3.8, 2.3, 1.2, T.hull, 1.05)
  b.box(-4.6, 2.3, -1.25, -3.8, 2.45, 1.25, T.lacquer)
  b.box(-4.0, 1.25, -1.38, 4.0, 1.4, -1.3, T.lacquer)
  b.box(-4.0, 1.25, 1.3, 4.0, 1.4, 1.38, T.lacquer)
  b.box(4.4, 1.2, -0.95, 4.7, 1.5, 0.95, '#f2eee6')
  b.box(4.45, 1.28, -0.97, 4.65, 1.44, 0.97, T.hullDark)
  b.box(-3.8, 1.3, -1.15, 3.9, 1.38, 1.15, T.deck)
  const mast = (x: number, h: number, w: number) => {
    b.box(x - 0.12, 1.35, -0.12, x + 0.12, 1.35 + h, 0.12, T.mast)
    b.box(x - w * 0.6, 1.35 + h * 0.3, -0.05, x + w * 0.4, 1.35 + h * 0.95, 0.05, T.sailRust)
    for (let y = 1.35 + h * 0.3 + 0.7; y < 1.35 + h * 0.95; y += 0.9) b.box(x - w * 0.62, y, -0.09, x + w * 0.42, y + 0.12, 0.09, T.batten)
  }
  mast(0.8, 8, 4.2)
  mast(3.2, 5.5, 2.6)
  for (let i = 0; i < 6; i++) b.box(-2.8 + i * 0.5, 0.7, 1.36, -2.62 + i * 0.5, 1.28, 1.42, '#6b675c')
  boatman(b, -4.2, 2.45, 0, '#5a6a78', false)
  return b.build()
}
