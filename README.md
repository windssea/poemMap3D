...
- **浏览器缓存与预热**：生成过的区块（网格与方块数据）存进 IndexedDB（`workers/ChunkStore.ts`），按构建号作废——构建号是世界生成源码与数据（`src/world`、`src/workers`、`src/config`、`src/utils`、掩膜、诗词地点）的内容指纹，源码不变缓存一直有效，改了就全部重算；开发时改到这些文件会自动重启开发服务器换新构建号，不会再用改动前的旧区块；Worker 空闲时预先生成名胜周围的区块（`world/ChunkWarmer.ts`）。第二次到同一处，平均每块生成耗时从约 6 毫秒降到约 1 毫秒。
...
- **环境**：晨昼暮夜 × 四季 × 晴雨雪，只改共享 uniform：光照、天空（方形的日；夜里一轮像素满月带月海与月晕、挂在月光同一方位的低空，两层明暗不一的星、亮星闪烁、一道淡银河）、线性雾、方块云、积雪、秋色、花期、雨雪与飘叶、瀑布水雾。夜里月光银白（约 35° 高、投影清楚），水面随波碎成一片月光银鳞、倒映星点；孔明灯与莲花河灯只在近景出现，镜头拉远渐渐淡去，远景留给星月。
...
- **诗人足迹**：屏幕空间粗线（远近同粗、抗锯齿，由暗到亮，外衬光晕）逐段画出，翻山处抬升先平滑成缓拱；镜头跟随笔头，到站题诗、说明卡与站点标记，意境随一生四季，结束后整体取景。每首诗都标了写作年份（确知的为实年，按行迹推定的为约数），到站配诗只取写于此站行迹之内的（说明里点名的优先），不会人还没到就题了后来的诗；卷上标「某年作」。
- **城池与江河**：带城墙的地标自动让开河湖；贴湖而过（或入湖前一段）的江河水面与湖面齐平，城中河渠在天然江湖岸边两格半内不开、留一道堤，岸边不再立起高低两层水面间的水墙；三峡段江面收窄、两岸起峭壁。
